...
async fn test_live_routing_round_robin() {
    ...
    eprintln!(
        "[routing] Round-robin results: {} successes, {} failures, {} rate-limited",
        success_count, failure_count, rate_limited_count
    );

    if success_count == 0 && rate_limited_count == num_requests {
        eprintln!("[SKIP] test_live_routing_round_robin: NVIDIA key budget exhausted (all 429)");
        return;
    }
...
async fn test_live_routing_latency_aware() {
    ...
    if latencies.is_empty() && rate_limited_count == test_models.len() {
        eprintln!("[SKIP] test_live_routing_latency_aware: NVIDIA key budget exhausted (all 429)");
        return;
    }
