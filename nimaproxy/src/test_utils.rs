...
mod tests {
	...
	// Test 12: Builder with duplicate models
	#[test]
	fn test_builder_with_duplicate_models() {
		let state = MockAppStateBuilder::new()
			.with_custom_racing_models(vec![
				"model-a".to_string(),
				"model-b".to_string(),
				"model-a".to_string(), // duplicate
			])
			.build();
		assert_eq!(state.racing_models.len(), 3);
	}

	// Test 13: Builder chain all methods
	#[test]
	fn test_builder_chain_all_methods() {
		let state = MockAppStateBuilder::new()
			.with_keys(5)
			.with_target("http://test:8080")
			.with_racing_models(3)
		...
		assert_eq!(state.racing_models.len(), 3);
		assert_eq!(state.racing_max_parallel, 4);
		assert_eq!(state.racing_timeout_ms, 10000);
		assert_eq!(state.racing_strategy, "latency-based");
	}

	// Test 14: create_test_state defaults
	#[test]
	fn test_create_test_state_defaults() {
		let state = create_test_state();
		assert_eq!(state.pool.len(), 1);
		assert_eq!(state.target, "http://127.0.0.1:8080");
		assert_eq!(state.racing_max_parallel, 3);
		assert_eq!(state.racing_timeout_ms, 5000);
		assert_eq!(state.racing_strategy, "round-robin");
	}

	// Test 15: create_racing_scenario validation
	#[test]
	fn test_create_racing_scenario_validation() {
		let state = create_racing_scenario(5, 3);
		assert_eq!(state.racing_models.len(), 5);
		assert_eq!(state.racing_max_parallel, 3);
		assert_eq!(state.racing_timeout_ms, 5000);
		assert_eq!(state.racing_strategy, "round-robin");
		// Verify models are correctly named
		assert!(state.racing_models[0].starts_with("nvidia/test-model-"));
	}
}
